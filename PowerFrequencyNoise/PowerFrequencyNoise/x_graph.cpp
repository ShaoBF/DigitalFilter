...
#define DGRID 20    //象素点数/每坐标格
    ...
    sprintf(str, "(%g,%g)", win2.x1, win2.y1);
    int nn = strlen(str);

    //printf("str=%s;nn=%d\n",str,nn);getch();

    TextOut(win2.hdc, win2.rect2.left, win2.rect2.bottom + 6, str, strlen(str));

    sprintf(str, "(%g,%g)", win2.x2, win2.y2);

    TextOut(win2.hdc, win2.rect2.right - 8 * strlen(str) + 14, win2.rect2.top - 18, str, strlen(str));

    sprintf(str, "%s:%g/格", ystr, DGRID / (win2.yk));
    TextOut(win2.hdc, win2.rect2.left, win2.rect2.top - 18, str, strlen(str));
    sprintf(str, "%s:%g/格", xstr, DGRID / (win2.xk));
    TextOut(win2.hdc, win2.rect2.right - strlen(str) * 8, win2.rect2.bottom + 6, str, strlen(str));
    ...
    sprintf(win2.xstr, xstr);
    sprintf(win2.ystr, ystr);
    //return 0;
...
void frame3(COLORREF bkcolor, COLORREF dotLineColor, char *xstr, char *ystr, char *zstr)
{
    ...
    m = getxy3();
    TextOut(win3.hdc, m.x, m.y, str, strlen(str));

    moveto3((win3.x1 + win3.x2) / 2, win3.y1, win3.z1);
    m = getxy3();
    TextOut(win3.hdc, m.x, m.y, xstr, strlen(xstr));

    moveto3(win3.x1, win3.y1, (win3.z1 + win3.z2) / 2);
    m = getxy3();
    TextOut(win3.hdc, m.x, m.y, zstr, strlen(zstr));

    SetTextAlign(win3.hdc, TA_LEFT | TA_TOP);
    moveto3(win3.x2, (win3.y1 + win3.y2) / 2, win3.z1);
    m = getxy3();
    TextOut(win3.hdc, m.x, m.y, ystr, strlen(ystr));
    ...
    sprintf(win3.xstr, xstr);
    sprintf(win3.ystr, ystr);
    sprintf(win3.zstr, zstr);
}
...
{
    //initgraph("信号处理工程训练",10,10,500,400);///
}
